...
<body>
<body
	style="background: url(images/bann.jpg) fixed; font-size: 14px !important;">
		...
		<section id="main-content" style="padding-top: 39px;">
				...
				<div class="row">
					<div class="col-lg-12">
						...
						<div
							...
							<%
								String them = (String) request.getAttribute("them");
								if (them != null) {
							%>
							<i class="far fa-smile-beam"
								style="color: red; margin-right: 6px;"></i><span
								style="color: red;"><%out.print(them);%>
							</span>
							<%
								}
							%>
			...
			<div>
				<table id="bang">
					...
					<c:forEach var="e" items="<%=dao.getAllDay_off()%>"
						varStatus="temp">
						<tr>
							<td>${temp.count}</td>
							<td>${e.getLast_Name()}${e.getFirst_Name()}</td>
							<td>${e.getBirthday()}</td>
							<c:if test="${e.getGender()==1}">
								<td>Nam</td>
							</c:if>
							...
							<td><form action="XuLyLuongNV" method="POST">
									<i class="fas fa-coins"> Lương Cơ Bản</i> <input type="text"
										name="Salary_Basic" required placeholder="Nhập Số"> <i
										class="fas fa-chart-line"> Tăng Ca</i> <input type="text"
										name="Overtime" required placeholder="Nhập Số"> <i
										class="fas fa-hand-holding-usd"> Phụ Cấp</i> <input
										type="text" name="Subsidize" required placeholder="Nhập Số">
									<input type="hidden" name="Employee_ID"
										value="${e.getEmployee_id()}"><input type="submit"
										value="Thanh Toán">
								</form></td>
						</tr>
					</c:forEach>
					<form action="XuLyLuongNV" method="GET">
						<i class="fas fa-wrench">Số ngày làm</i> <input type="text"
							name="Working_days" required placeholder="Nhập Số"> <input
